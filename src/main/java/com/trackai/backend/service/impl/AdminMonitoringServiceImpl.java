...
public class AdminMonitoringServiceImpl implements AdminMonitoringService {
    ...
    @Override
    public void recordRequest(HttpServletRequest request, int status, long durationMs, String contentType, long responseBytes) {
    ...
    @Override
    public AdminRequestLogPageResponse requestLogs(
            int requestedPage,
            int requestedSize,
            String user,
            Integer status,
        ...
        int size = Math.min(100, Math.max(10, requestedSize));
        String userFilter = user == null ? "" : user.trim().toLowerCase(Locale.ROOT);
        String pathFilter = path == null ? "" : path.trim().toLowerCase(Locale.ROOT);

        List<RequestSnapshot> filtered = recentRequests(retentionCutoff(Instant.now())).stream()
                .filter(item -> userFilter.isBlank()
                        || item.userIdentity().toLowerCase(Locale.ROOT).contains(userFilter))
                .filter(item -> status == null || item.status() == status)
                .filter(item -> pathFilter.isBlank()
                        || item.path().toLowerCase(Locale.ROOT).contains(pathFilter))
                .toList();
        ...
        return new AdminRequestLogPageResponse(
                filtered.subList(from, to).stream().map(this::toEntry).toList(),
    ...
    private AdminMonitoringResponse.RequestEntry toEntry(RequestSnapshot item) {
        return new AdminMonitoringResponse.RequestEntry(
                ...
                item.clientIp(), item.location(), item.userAgent(), item.userIdentity(),
                responseSummary(item.status()), item.contentType(), item.responseBytes());
    }

    private String responseSummary(int status) {
        if (status >= 500) return "Server error";
        if (status >= 400) return "Request rejected";
        if (status >= 300) return "Redirected";
        if (status == 204) return "Completed without response body";
    ...
    private record RequestSnapshot(
            ...
            String contentType, long responseBytes) {
    }
